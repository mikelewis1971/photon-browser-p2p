import { generateKeypair, importPrivateKey, importPublicKey, sha256, generateECDHKeypair, importECDHPrivateKey, importECDHPublicKey } from './crypto.js';
...
export async function getIdentity() {
  ...
  try {
    ...
    // Auto-upgrade identity with ECDH keys if missing
    if (!id.ecdhPublicKeyB64) {
      const ecdhKp = await generateECDHKeypair();
      id.ecdhPublicKeyB64 = ecdhKp.ecdhPublicKeyB64;
      id.ecdhPrivateKeyB64 = ecdhKp.ecdhPrivateKeyB64;
      saveIdentity(id);
    }
    ...
    id.ecdhPublicKey = await importECDHPublicKey(id.ecdhPublicKeyB64);
    id.ecdhPrivateKey = await importECDHPrivateKey(id.ecdhPrivateKeyB64);
    return id;
  } catch (e) {
...
export async function createNewIdentity(displayName = 'Anonymous', userHandle = '') {
  ...
  const identity = {
    fingerprint,
    displayName,
    userHandle: userHandle || fingerprint.substring(0, 8),
    publicKeyB64: kp.publicKeyB64,
    privateKeyB64: kp.privateKeyB64,
    ecdhPublicKeyB64: ecdhKp.ecdhPublicKeyB64,
    ecdhPrivateKeyB64: ecdhKp.ecdhPrivateKeyB64,
    bio: '',
    following: [],
    blocked: [],
    authorizedPeers: [], // New whitelist for private access
    seq: 0,
    createdAt: Date.now()
  };
...
export function saveIdentity(identity) {
  ...
  delete toStore.ecdhPublicKey;
  delete toStore.ecdhPrivateKey;
...
export async function importIdentityFromJson(jsonStr) {
  try {
    ...
    const identity = {
      ...
      userHandle: idData.userHandle || idData.fingerprint?.substring(0, 8) || 'user',
      ...
      authorizedPeers: idData.authorizedPeers || [],
      seq: idData.seq || 0,
