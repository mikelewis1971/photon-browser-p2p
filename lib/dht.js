import { importPublicKey, verify } from './crypto.js';
...
class DHT {
  ...
  _announce() { 
    ...
    this._broadcast({
      ...
      ecdhPublicKeyB64:this.identity.ecdhPublicKeyB64,
      timestamp:Date.now()
    }); 
  }
  _broadcast(msg) { 
  ...
  async _handleMessage(msg) {
    ...
    if(msg.type==='new_message') {
      const peer = this.peers.get(msg.message.author);
      if (peer && peer.publicKeyB64 && msg.message.signature) {
        const raw = { text: msg.message.text, timestamp: msg.message.timestamp, author: msg.message.author, displayName: msg.message.displayName };
        const pubKey = await importPublicKey(peer.publicKeyB64);
        const isValid = await verify(pubKey, JSON.stringify(raw), msg.message.signature);
        if (isValid) {
          // If we haven't seen this hash, store it to relay
          const key = 'msg_' + msg.message.hash;
          if (!this.get(key)) {
            this.putMessage(msg.message); // This will _localPut and re-emit locally
          }
        } else {
          console.warn('Forged feed post dropped');
        }
      }
    }
  ...
  async requestAccess(targetFingerprint) {
    const msg = {
      type: 'access_request',
      from: this.identity?.fingerprint,
      displayName: this.identity?.displayName,
      handle: this.identity?.userHandle,
      timestamp: Date.now()
    };
    this.signal(targetFingerprint, msg);
  }
