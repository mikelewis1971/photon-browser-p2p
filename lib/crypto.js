...
export async function deriveSharedSecret(ecdhPrivateKey, ecdhPublicKeyB64) {
  const pubKey = await importECDHPublicKey(ecdhPublicKeyB64);
  const bits = await crypto.subtle.deriveBits(
    { name: 'ECDH', public: pubKey },
    ecdhPrivateKey,
    256
  );
  return bufToBase64(bits); // Use as AES key material
}
...
export async function generateECDHKeypair() {
  ...
  const pub = await crypto.subtle.exportKey('spki', kp.publicKey);
  const priv = await crypto.subtle.exportKey('pkcs8', kp.privateKey);
  return { ecdhPublicKey: kp.publicKey, ecdhPrivateKey: kp.privateKey, ecdhPublicKeyB64: bufToBase64(pub), ecdhPrivateKeyB64: bufToBase64(priv) };
}
...
export async function importECDHPrivateKey(b64) {
  const buf = base64ToBuf(b64);
  return crypto.subtle.importKey('pkcs8', buf, {name:'ECDH',namedCurve:'P-256'}, true, ['deriveKey', 'deriveBits']);
}
...
export async function importECDHPublicKey(b64) {
  const buf = base64ToBuf(b64);
