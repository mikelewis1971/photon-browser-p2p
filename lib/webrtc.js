...
export class PeerConnection {
  ...
  _setupDataChannel() {
    this.dataChannel.onopen = () => {
      console.log('P2P connection open with', this.targetId);
      // Announce our local identity immediately to the new remote peer
      dht._announce(); 
    };
    this.dataChannel.onmessage = (e) => {
      try {
        const msg = JSON.parse(e.data);
        // Route remote WebRTC message directly into local DHT engine
        dht._handleMessage(msg);
      } catch (err) {
        console.error('Invalid P2P message format', err);
      }
    };
  }
...
export const webrtc = {
  ...
  init() {
    ...
    // Forward all local DHT broadcasts over WebRTC
    dht.on('broadcast', (msg) => this.broadcast(msg));
  },
